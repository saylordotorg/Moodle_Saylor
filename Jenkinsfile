...
def moodle_version = 'fix/registration-cache-issue'
...
def StashMoodle(moodle_version) {
    node {
        deleteDir()
            try {
                git([url: 'https://github.com/saylordotorg/moodle.git', branch: "${moodle_version}"])
            }
            catch(err) {
...
def CopyDatabase(mysql_user, mysql_password, mysql_source_host, mysql_source_dbname, mysql_dest_dbname) {
    ...
    withCredentials([string(credentialsId: 'mysql-dev-01_host', variable: 'mysql_dest_host')]) {
        ...
        sh "mysqldump --single-transaction --ignore-table=mdl_logstore_standard_log --host ${mysql_source_host} -u ${mysql_user} --password=${mysql_password} ${mysql_source_dbname} | mysql -h ${mysql_dest_host} -u ${mysql_user} --password=${mysql_password} ${mysql_dest_dbname}"
    }
